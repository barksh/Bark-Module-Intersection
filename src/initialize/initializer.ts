...
import { INTERSECTION_MONGO_DB, INTERSECTION_SELF_DOMAIN } from "./environment";
...
export class Initializer {
    ...
    private async initialize(): Promise<void> {
        ...
        if (!INTERSECTION_MONGO_DB) {

            throw panic.code(
                ERROR_CODE.ENVIRONMENT_VARIABLE_REQUIRED_BUT_NOT_FOUND_1,
                'INTERSECTION_MONGO_DB',
            );
        }

        if (!INTERSECTION_SELF_DOMAIN) {

            throw panic.code(
                ERROR_CODE.ENVIRONMENT_VARIABLE_REQUIRED_BUT_NOT_FOUND_1,
                'INTERSECTION_SELF_DOMAIN',
            );
        }
        ...
        try {
            ...
            const connection: Mongoose.Connection = await connectDatabase(INTERSECTION_MONGO_DB);

            logAgent.debug("Initialized");

            this._connection = connection;
            this._selfDomain = INTERSECTION_SELF_DOMAIN;
